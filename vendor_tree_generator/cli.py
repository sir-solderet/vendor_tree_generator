...
@click.command()
@click.option('--super', '-s', 'super_img', 
              type=click.Path(exists=True), 
              help='Path to super.img file')
@click.option('--partition', '-p', 'partition_img',
              type=click.Path(exists=True),
...
@click.option('--vendor', '-v', 'vendor_name',
              required=True,
              help='Vendor name (e.g., samsung)')
@click.option('--device', '-d', 'device_name',
              required=True,
              help='Device codename (e.g., gta9)')
@click.option('--android-version', '-av', 'android_version',
              default='13',
...
def main(super_img, partition_img, output_dir, vendor_name, device_name, 
         android_version, verbose, dry_run):
    ...
    if not check_dependencies():
        logger.error("Missing required dependencies. Please install lpunpack, simg2img, and mount tools.")
        sys.exit(1)
