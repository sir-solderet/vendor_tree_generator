...
class VendorTreeGenerator:
    ...
    def extract_image(self, image_path: Path):
        ...
        print(f"[DEBUG] Extracting image: {image_path.name}")
        ...
        try:
            ...
            self.logger.warning("7z extraction failed: %s", e)
            print(f"[DEBUG] 7z extraction failed for {image_path.name}")
            # TODO: Implement lpunpack fallback if needed

        self.logger.info("Extracted to %s", extract_dir)
        print(f"[DEBUG] Extracted contents to: {extract_dir}")
    ...
    def generate_tree(self, extracted_path: str, output_dir: str) -> bool:
        try:
            ...
            self.logger.info("Generated vendor tree with %d proprietary files", len(self.proprietary_files))
            return True
    ...
    def _scan_proprietary_files(self, extracted_path: str):
        ...
        for root, _, files in os.walk(extracted_path):
            for file in files:
                ...
                print(f"[DEBUG] Checking file: {rel_path}")
        ...
        print(f"[DEBUG] Found {count} proprietary files")
        self.logger.info("Found %d proprietary files", count)
    ...
    def _copy_proprietary_files(self, extracted_path: str, output_dir: str):
        ...
        for file_info in self.proprietary_files:
            ...
            try:
                ...
                print(f"[DEBUG] Failed to copy {rel_path}: {e}")
